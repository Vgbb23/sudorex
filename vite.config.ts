...
export default defineConfig(({mode}) => {
  ...
  const fruitfyReady = Boolean(fruitfyConfig.token && fruitfyConfig.storeId && fruitfyConfig.productId);
  if (!fruitfyReady) {
    console.warn(
      '\n[Fruitfy] PIX desativado: crie .env.local com FRUITFY_TOKEN, FRUITFY_STORE_ID e FRUITFY_PRODUCT_ID (veja .env.example).\n'
    );
  }
